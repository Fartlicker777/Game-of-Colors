...
public class GameOfColors : MonoBehaviour {
    ...
    void SubmitPress () {
        ...
        if (Wrong) {
            Debug.LogFormat("[Game of Colors #{0}] Incorrect submission:", moduleId);
            for (var x = 0; x < 5; x++)
                Debug.LogFormat("[Game of Colors #{0}] {1}", moduleId, Submission.Skip(5*x).Take(5).Select(a => "KRGYBMCW"[7 - a]).Join(""));
            Debug.LogFormat("[Game of Colors #{0}] Following cells are incorrectly colored: {1}", moduleId, WrongCoords.Join(", "));
...
#pragma warning restore 414
    ...
    private IEnumerator ProcessTwitchCommand (string command) {
        ...
        if (command == "submit") {
            yield return null;
            Submit.OnInteract();
            yield return FinalAnswer.SequenceEqual(Submission) ? "solve" : "strike";
            yield break;
        }
